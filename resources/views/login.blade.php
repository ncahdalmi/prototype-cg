    ...
    <form action="/{{ Request::is('register') ? 'register' : 'login' }}"
        ...
        @if (Request::is('register'))
            <input type="email" name="email" autocomplete="off"
                style="text-align: center; width:100%; display:block; padding: 10px;" placeholder="email" value="{{ old('email') }}">
                @error('email')
                    <p>{{ $message }}</p>
                @enderror
            <input type="text" name="name" autocomplete="off"
                style="text-align: center; width:100%; display:block; padding: 10px;" placeholder="name"  value="{{ old('name') }}">
                @error('name')
                    <p>{{ $message }}</p>
                @enderror
        @endif
        ...
        <input style="text-align: center; width:100%; display:block; padding: 10px;" type="password" name="password"
            ...
            @error('password')
                ...
                @enderror
        @if (Request::is('register'))
            <input style="text-align: center; width:100%; display:block; padding: 10px;" type="password" name="retype_password"
                id="retype_password" placeholder="retype password"  value="{{ old('retype_password') }}">
